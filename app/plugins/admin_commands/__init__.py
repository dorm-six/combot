...
class AdminCommands:
    ...
    def is_ok(msg):
        ...
        if cmd_obj.is_semicolon() and cmd_obj.is_cmd_eq("pin"):
            ...
            cmd_obj.is_param_semicolon()
            and cmd_obj.is_cmd_eq("msg")
            and cmd_obj.param.isdigit()
            and len(cmd_obj.value) > 0
        ):
            return True
        else:
            return False
            ...
            if API.sendMsg(send_to_chat_id, cmd_obj.value):
                API.sendMsg(
                    ...
                    'SUCCESSFULLY SENT "{}" to "{}"'.format(
                        cmd_obj.value, send_to_chat_id
                    ),
                )
            else:
                API.sendMsg(
                    CHAT_ID_SUPERUSER,
                    'NOT SENT "{}" to "{}"'.format(cmd_obj.value, send_to_chat_id),
                )
            return True
